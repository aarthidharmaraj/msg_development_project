...
class MoveSftpToS3Local:
    ...
    def get_latest_files_from_local(self, local_path, source_path, sftp_path):
        ...
        try:
            ...
            latest_files = list(set(temp_sftp_files).difference(set(temp_s3_files)))
    ...
    def upload_zip_text_file_to_given_path(
        self, local_path, partition_path, sources, file_name, temp_s3_path
    ):
        """This method takes the zip source and filename and upload the zip to
        source path, extracted text file to stage path."""
        try:
            self.logger.info(
                "Uploading the file %s from sftp server to the given path %s",
                file_name,
                temp_s3_path,
            )
            file = self.upload_partition_s3_local(
                local_path + "/" + temp_s3_path,
                sources,
                file_name,
                partition_path,
            )
            ...
            self.logger.error(
                "Cannot upload the file %s to the given path %s , %s", file_name, temp_s3_path, err
            )
            file = None
        return file

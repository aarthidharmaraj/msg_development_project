"""This module gets the response from api with authentication of api key by decrypting it
from cinfig for the given endpoints with pagination"""
import requests
import pandas as pd
...
class PullDataFromFreeSoundApi:
    """This class has methods to pull similar sounds for soundid, user packs and user
    sounds for given username from api with authentication by decrypting api key from config"""
    ...
    def fetch_user_sounds_packs_from_api(self, username):
        """This method fetches the user sounds or user packs based on the endpoint value
        by creating an endpoint"""
        try:
            endpoints = f"users/{username}/packs/"
            self.logger.info("Getting the user pack response from api with endpoint %s", endpoints)
    ...
    def get_response_from_api_pagination(self, endpoint):
        """This method gets the response from api for the given endpoints with pagination using
        the next link"""
        try:
            chunk_data = []

"""This module fetches Similar sounds, user_packs from freesound api for the provided sound id and username partition them based on the created date and upload them to s3 in the given path """
...
class FetchDataFromApiUploadS3:
    """This class has methods to fetch datas from api based on the endpoints for the given sound id and username, partition them based on date and upload them to s3"""
    ...
    def fetch_response_from_api_for_endpoints(self):
        ...
        try:
            if self.endpoint =='similar_sound' and self.soundid:#checks for endpoint
                self.logger.info("Fetching response for similar sounds from api for sound id %s",self.soundid)
                ...
                self.logger.info("Fetching response for %s from api for username %s",self.endpoint,self.username)
                dataframe=self.api.fetch_user_sounds_packs_from_api(self.username)
                date=self.get_date_from_dataframe(dataframe)
            else:
                self.logger.error("The given endpoint doesn't match with thr eequired endpoints")
    ...
    def get_date_from_dataframe(self,dataframe):
        """This method gets the date from dataframe and create json file and make partition based on the created date"""
        try:
    ...
    def put_partition_path(self,date):
        ...
        try:
            ...
            self.logger.info(
                "Made the partition based on date %s", partition_path
            )
        except Exception as err:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This argparser is to get sound id, endpoint, username for fetching the datas from api"
    )
    parser.add_argument(
        "--soundid",

"""This method get the response from holiday api for the given year and region based on their status codes"""
import requests

class HolidayApi:
    """This class has methods to get details from Holiday Api for the given endpoints, region and year"""
    ...
    def check_availability_of_year(self,year):
        ...
        try:
            ...
            available_year=None
            self.logger.error("%s is not available for getting public holidays from api",year)
    ...
    def get_endpoint_for_public_holidays_long_weekend(self,year,country,epoint):
        ...
        try:
            ...
            self.logger.error("Cannot get the %s endpoint for %s and %s %s",epoint,year,country,err)
            response=None
        return response
    
    def get_endpoint_for_next_publicholidays(self,country):
        """This method gets the endpoint for fetching the public holidays for next 365 days for given country
        parameters:  country - region for which datas are needed"""
        try:
            endpoint=f"NextPublicHolidays/{country}"
            self.logger.info("Got next_publicholidays endpoint for given year and region %s",endpoint)
            self.get_response_from_api("next holiday",endpoint)
        except Exception as err:
            self.logger.error("Cannot get the next_publicholidays endpoint for %s %s",country,err)
            endpoint=None
        return endpoint
    
    # def get_endpoint_for_long_weekend(self,year,country):
    #     """This method gets the endpoint for fetching the Long weekend for given year and country
    #     parameters: year - year on which long weekend details are needed
    #                 country - region for which datas are needed"""
    #     try:
    #         endpoint=f"LongWeekend/{year}/{country}"
    #         self.logger.info("Got LongWeekend endpoint for given year and region %s",endpoint)
    #         response=self.get_response_from_api(endpoint)
    #     except Exception as err:
    #         self.logger.error("Cannot get the LongWeekend endpoint for %s and %s %s",year,country,err)
    #         response=None
    #     return response
    ...
    def get_response_from_api(self,endpoint):
        ...
        try:
            ...
            if response.status_code!=200:
                ...
                self.logger.info(
                    "Got the response from api for given year with status %s",response.status_code,
                )
        except Exception as err:

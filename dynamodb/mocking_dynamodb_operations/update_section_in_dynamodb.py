...
for that section, and update them in dynamodb table with mocked dynamodb"""
...
class ConfigDynamoDB:
    ...
    def update_item_args(self,section_name):
        """This method returns the arguments for updating item in dynamodb"""
        parameters={'Key':{'section_name':section_name},'UpdateExpression':"SET s3_path= :s",'ExpressionAttributeValues':{':s': 'source/example'},'ReturnValues':"UPDATED_NEW"}
        return parameters
    ...
    def update_to_dynamodb(self,section_data,section_name):
        ...
        try:
            ...
            print(get_data)
            # update_params=self.update_item_args(section_name)
            # update_data=self.dynamodb.update_item_from_table(self.table_name,table_params,put_params,update_params)
            # get_data=self.dynamodb.get_item_from_table(self.table_name,table_params,put_params,get_params)
            # print(get_data)

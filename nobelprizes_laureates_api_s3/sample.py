    ...
    def get_employee_details_for_givendates(self):
        ...
        if self.enddate:
            if self.startdate >= (datetime.now().date()) or self.enddate >=(datetime.now().date()):
                self.logger.info("Cannot fetch the details for current and future dates")
                sys.exit("script was terminated when fetching for current and future dates")
            else:
